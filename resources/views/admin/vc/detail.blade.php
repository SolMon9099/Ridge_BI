...
<form action="{{route('admin.vc.detail')}}" method="get" name="form1" id="form1">
@csrf
    <input type="hidden" name="change_params" value="change"/>
    <div id="wrapper">
        <div class="breadcrumb">
        <ul>
            <li><a href="{{route('admin.vc.detail')}}">車両エリア侵入検知</a></li>
            <li>TOP</li>
        </ul>
        </div>
        <div id="r-content">
            <div class="title-wrap">
                <h2 class="title">TOP(車両エリア侵入検知)</h2>
            </div>
                ...
                <div class="inner active">
                    <div style="display: flex;">
                        <div style="width:50%; position: relative;">
                            ...
                            <button type="button" class="add-to-toppage <?php echo $from_top?'from_top':'' ?>" onclick="addDashboard({{config('const.top_block_type_codes')['live_video_vc']}})">ダッシュボートへ追加</button>
                        </div>
                        <div style="width:50%; position: relative;">
                            <h3 class="title">当日グラフ</h3>
                            <button type="button" class="add-to-toppage <?php echo $from_top?'from_top':'' ?>" onclick="addDashboard({{config('const.top_block_type_codes')['live_graph_vc']}})">ダッシュボートへ追加</button>
                        </div>
                    </div>
                    <div class="mainbody">
                        ...
                        <canvas id="myLineChart1"
                            onclick="location.href='{{route('admin.vc.past_analysis').'?time_period=time&change_params=change&starttime='.date('Y-m-d').'&endtime='.date('Y-m-d').'&selected_rule='.$selected_rule->id}}'">
                        </canvas>
                    </div>
                </div>
            </div>
            ...
            <div style="position: relative;">
                ...
                <button type="button" class="add-to-toppage <?php echo $from_top?'from_top':'' ?>" style="top:0;"
                    onclick="addDashboard({{config('const.top_block_type_codes')['recent_detect_vc']}})">ダッシュボートへ追加
                </button>
                @endif
            </div>
            @if(count($vc_detections) == 0)
                ...
                <li>
                    ...
                    <div class="text">
                        ...
                        <ul class="pit-list">
                            ...
                            <li><a class="move-href" href="{{route("admin.vc.list")}}">検知リスト</a></li>
                        </ul>
                    </div>
                </li>
                @endforeach
            ...
            <div class="listing">
                <div class="scroll active sp-pl0">
                    <table class="table2 text-centre">
                        ...
                        @if(count($cameras) == 0)
                        <tr>
                            <td colspan="6">車両エリア侵入検知のルールが登録されたカメラがありません。ルールを設定してください</td>
                        </tr>
                        @endif
                        </tbody>
                    </table>
...
<script>
    ...
    function displayGraphData(e = null, x_range = '3'){
        ...
        saveSearchOptions('admin.vc.detail', search_params);
    }
