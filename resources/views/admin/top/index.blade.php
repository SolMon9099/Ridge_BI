            ...
            <div class="title-area">
                ...
                <ul class="user-menu">
                    <li style="cursor: pointer">
                        <a class="edit_status_change" onclick="changeEditStatus()">
                            {{$edit_enable_flag ? 'ビューモードにする' : '編集モードにする'}}
                        </a>
                    </li>
                    ...
                    <div class="grid-stack-item" data-id = "{{$item->id}}" gs-x="{{$item->gs_x}}" gs-y="{{$item->gs_y}}" gs-w="{{$item->gs_w}}" gs-h="{{$item->gs_h}}">
                        <div class="grid-stack-item-content">
                            ...
                            <div class="live-stream-title">
                                <img class="extension-icon" onclick="showExtensionModal(this, '{{config('const.top_block_titles')[$item->block_type]}}')"
                                    src="{{asset('assets/admin/img/search_icon.png')}}" style="display:'<?php echo $edit_enable_flag ? "none": "block" ?>'"/>
                                {{config('const.top_block_titles')[$item->block_type]}}
                            </div>
...
<script>
    var edit_enable_flag = "<?php echo $edit_enable_flag;?>";
    ...
    if (edit_enable_flag == false){
        $('.grid-stack-item').removeClass('ui-draggable');
        $('.grid-stack-item').removeClass('ui-resizable');
        $('.grid-stack-item').removeClass('ui-resizable-autohide');
        $('.grid-stack-item').addClass('ui-draggable-disabled');
        $('.grid-stack-item').addClass('ui-resizable-disabled');
        $('.grid-stack-item').addClass('ui-resizable-autohide');
    }
    var stages = {};
    ...
    function changeEditStatus()
    {
        $('.user-menu').hide();
        $('.total-menu-letter').html('編集');
        edit_enable_flag = !edit_enable_flag;
        if (edit_enable_flag == false){
            $('.edit_status_change').html('編集モードにする');
            $('.extension-icon').show();
            $('.grid-stack-item').removeClass('ui-draggable');
            $('.grid-stack-item').removeClass('ui-resizable');
            $('.grid-stack-item').removeClass('ui-resizable-autohide');
            ...
            $('.edit_status_change').html('ビューモードにする');
            $('.extension-icon').hide();
            $('.grid-stack-item').addClass('ui-draggable');
            $('.grid-stack-item').addClass('ui-resizable');
            $('.grid-stack-item').addClass('ui-resizable-autohide');
            $('.grid-stack-item').removeClass('ui-draggable-disabled');
            $('.grid-stack-item').removeClass('ui-resizable-disabled');
            $('.grid-stack-item').removeClass('ui-resizable-autohide');
                ...
                if (detect_time_object.getTime() >= cur_time.getTime() && detect_time_object.getTime() < cur_time.getTime() + grid_unit * props * 60 * 1000){
                    ...
                    Object.keys(actions).map(id => {
                        if (graph_data[detect_time][id] != undefined){
                            totals_by_action[id].push(graph_data[detect_time][id].length);
                            if (graph_data[detect_time][id].length > max_y) max_y = graph_data[detect_time][id].length;
                        } else {
                            totals_by_action[id].push(0);
                        }
                    })
