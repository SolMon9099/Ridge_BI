...
(function ($) {
  ...
  $.wop.miniCalendar.prototype = {
    ...
    printType: function (thisYear, thisMonth) {
      ...
      // 開始の曜日
      var startWeek = thisDate.getDay() - 1;
     ...
     var rowMax = Math.ceil((lastday + startWeek) / 7);
      ...
      function printTD(count, col) {
        ...
        // 曜日classを割り当てる
        if (col === 6) tmpId += ' class="calendar-sun"';
        if (col === 5) tmpId += ' class="calendar-sat"';
      ...
      for (var i = 0; i < this.events.length; i++) {
        ...
        if (this.events[i].type) {
          type = ' ' + this.events[i].type;
							...
							var type2 = "";
        if (this.events[i].type2) {
          type2 = '' + this.events[i].type2;
        }
       var type3 = "";
        if (this.events[i].type3) {
          type3 = '' + this.events[i].type3;
        }
        $(this.ele)
          .find('#' + dateID + ' .calendar-labels').append('<a class="calender-label' + type + '" href="' + type2 + '" data-target="' + type3 + '" >' + getText.val() + '</a>');
