...
(function(global, factory) {
    ...
    function toType(obj) {
        if (obj === window && obj.document && obj.location) { return 'window'; } else if (obj === document) { return 'htmldocument'; } else if (typeof obj === 'string') { return 'string'; } else if (isElement(obj)) { return 'element'; }
    }
    ...
    Html.prototype.createPicker = function createPicker() {
        ...
        for (; u < 4; u++) {
            ...
            for (; i < ii; i++) {
                if (i < 10) {
                    hours_html.push(
                        ['<li><a ', VARS.attr.hour, '="', '0' + i, '">', i, '</a></li>'].join('')
                    );
                } else {
                    hours_html.push(
