    ...
    static getCameras() {
        return new Promise((resolve, reject) => {
            if (navigator.mediaDevices
                ...
                this._log("サポートされているデバイスにアクセスできません。");
                reject("サポートされているデバイスにアクセスできません。");
                return false;
            }
        });
    }
...
class Html5QrcodeScanner {
    ...
    render(qrCodeSuccessCallback, qrCodeErrorCallback) {
        ...
        this.qrCodeSuccessCallback = message => {
            $this.__setStatus("マッチ", Html5QrcodeScanner.STATUS_SUCCESS);
            if (qrCodeSuccessCallback) {
    ...
    __createBasicLayout(parent) {
        ...
        qrCodeScanRegion.classList.add('qr-scan-region');
    ...
    __createHeader(dashboard) {
        ...
        header.id = this.__getHeaderSectionId();
        header.classList.add('qr-header-section');
        dashboard.appendChild(header);

       /* const titleSpan = document.createElement("span");
        titleSpan.id = this.__getHeaderTitleId();
        titleSpan.innerHTML = "QRコードスキャナー";
        titleSpan.classList.add('qr-header-title');
        header.appendChild(titleSpan);*/

        const statusSpan = document.createElement("span");
        statusSpan.id = this.__getStatusSpanId();
        statusSpan.classList.add('qr-header-status');
        header.appendChild(statusSpan);
        this.__setStatus("待ち");

        const headerMessageContainer = document.createElement("div");
        headerMessageContainer.id = this.__getHeaderMessageContainerId();
        headerMessageContainer.classList.add('qr-header-message');
    ...
    __createSectionControlPanel() {
        ...
        requestPermissionButton.innerHTML = "カメラ権限リクエスト";
        requestPermissionButton.classList.add('qr-section-btn');
        requestPermissionButton.classList.add('qr-request-camera-permission');
        requestPermissionButton.addEventListener("click", function (e) {
            e.preventDefault();
            requestPermissionButton.disabled = true;
            $this.__setStatus("許可");
            $this.__setHeaderMessage("カメラの権限をクエスト中...");
    ...
    __renderCameraSelection(cameras) {
        ...
        cameraActionStartButton.innerHTML = "スキャンの開始";
        cameraActionStartButton.classList.add('qr-start-scan');
        cameraActionStartButton.classList.add('qr-section-btn');
        cameraActionContainer.appendChild(cameraActionStartButton);

        const cameraActionStopButton = document.createElement("button");
        cameraActionStopButton.innerHTML = "スキャンの停止";
        cameraActionStopButton.classList.add('qr-start-scan');
        cameraActionStopButton.classList.add('qr-section-btn');
        ...
        cameraActionStopButton.addEventListener('click',  function (e){
            e.preventDefault();
            cameraActionStopButton.disabled = true;
    ...
    __createSectionSwap() {
        ...
        swithToFileBasedLink.disabled = true;
        ...
        swithToFileBasedLink.addEventListener('click', function (e) {
            e.preventDefault();
    ...
    __getHeaderSectionId() {
        return `${this.elementId}__header_section`;
    }

    __getHeaderTitleId() {
        return `${this.elementId}__header_title`;
    }
