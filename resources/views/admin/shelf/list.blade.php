...
<div id="wrapper">
    ...
    <div id="r-content">
        ...
        @if(count($shelf_detections) > 0)
            <button type="button" class="add-to-toppage <?php echo isset($from_top) && $from_top == true ?'from_top':'' ?>" onclick="addDashboard({{config('const.top_block_type_codes')['detect_list_shelf']}})">ダッシュボートへ追加</button>
        @endif
        @if(!(count($shelf_detections) > 0))
