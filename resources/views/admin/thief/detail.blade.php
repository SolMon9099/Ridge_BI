...
<form action="{{route('admin.thief.detail')}}" method="get" name="form1" id="form1">
@csrf
        ...
        <div id="r-content">
                ...
                <div class="inner active">
                    <div style="display: flex;">
                        <div style="width:50%; position: relative;">
                            ...
                            <button type="button" class="add-to-toppage <?php echo $from_top?'from_top':'' ?>" onclick="addDashboard({{config('const.top_block_type_codes')['live_video_thief']}})">ダッシュボートへ追加</button>
                        </div>
                        <div style="width:50%; position: relative;">
                            <h3 class="title">当日グラフ</h3>
                            <button type="button" class="add-to-toppage <?php echo $from_top?'from_top':'' ?>" onclick="addDashboard({{config('const.top_block_type_codes')['live_graph_thief']}})">ダッシュボートへ追加</button>
                        </div>
                    </div>
                    <div class="mainbody">
                        <div class='video-show' style="width:54%;padding-top:40px;">
                            <div id="image-container" onclick="location.href='{{route('admin.thief.edit', ['thief' => $selected_rule->id])}}'"></div>
                        ...
                        <canvas id="myLineChart1"
                            onclick="location.href='{{route('admin.thief.past_analysis').'?time_period=time&change_params=change&starttime='.date('Y-m-d').'&endtime='.date('Y-m-d').'&selected_rule='.$selected_rule->id}}'">
                        </canvas>
                    </div>
                </div>
            </div>
            ...
            <div style="position: relative;">
                ...
                <button type="button" class="add-to-toppage <?php echo $from_top?'from_top':'' ?>" style="top:0;"
                    onclick="addDashboard({{config('const.top_block_type_codes')['recent_detect_thief']}})">ダッシュボートへ追加
                </button>
                @endif
            </div>
            @if(count($thief_detections) == 0)
                        ...
                        <ul class="pit-list">
                            ...
                            <li>
                                ...
                                <p style="margin-top: 10px;">
                                    <a class="move-href" href="{{route("admin.thief.list")}}">検知リスト</a>
                                </p>
                            </li>
                        </ul>
                    </div>
...
<script>
    ...
    function displayGraphData(e = null, x_range = '3'){
        ...
        saveSearchOptions('admin.thief.detail', search_params);
    }
